...
Client.prototype.authorizeUrl = function () {
    return 'https://my.locative.io/oauth2/authorize?client_id=' + this.client_id + '&response_type=code&redirect_uri=' + this.redirect_uri;
}

Client.prototype.getToken = function (code, cb) {
    request.post({
        headers: {'content-type' : 'application/x-www-form-urlencoded'},
        url: 'https://my.locative.io/oauth2/token',
...
Client.prototype.getFencelogs = function (token, cb) {
    request.get({
        ...
        url: 'https://my.locative.io/api/v1/fencelogs'
    }, function (error, response, body) {
        var json = null;
        if (!error && body !== 'Unauthorized') {
            try {
